  ...
  return (
    ...
    <div className="h-[100dvh] w-full overflow-y-auto overscroll-y-contain bg-[#1c1c1e] text-zinc-100 p-3 sm:p-6 md:p-8 font-sans flex flex-col items-center pb-32">
      ...
      <div className="w-full max-w-4xl bg-zinc-900 border border-zinc-800 rounded-2xl md:rounded-[2rem] p-4 sm:p-6 md:p-8 shadow-inner">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-5 place-items-center">
      ...
      {/* --- DESKTOP POPOVERS (Appears on desktop only) --- */}
      {showModal && !isMobile && selectedAward && (() => {
        const awardIndex = AWARDS_DB.findIndex(a => a.id === selectedAward.id);
        const gridCols = window.innerWidth >= 1280 ? 5 : window.innerWidth >= 768 ? 5 : window.innerWidth >= 640 ? 4 : 3;
        const column = awardIndex % gridCols;
        const isLeftColumn = column === 0;
        const isRightColumn = column === gridCols - 1;
